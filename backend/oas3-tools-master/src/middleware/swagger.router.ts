...
export class SwaggerRouter {
          ...
          try {
            ...
            (async () =>
              (controller = await import(
                path.resolve(path.join(dir, controllerName))
              )))();
          }
  ...
  initialize(options) {
    ...
    console.log(
      "  Mock mode: %s",
      options.useStubs === true ? "enabled" : "disabled"
    );
    ...
    const getHandlerName = (req) => {
      if (req.openapi.schema["x-swagger-router-controller"]) {
        let operationId = req.openapi.schema.operationId
          ? req.openapi.schema.operationId
          : req.method.toLowerCase();
        operationId = removeDashElementToCamelCase(operationId);
        return (
          req.openapi.schema["x-swagger-router-controller"] +
          "_" +
          operationId
        ).toLocaleLowerCase();
      } else {
    ...
    const send405 = (req, res, next) => {
      let err = new Error(
        "Route defined in OpenAPI specification (" +
          req.openapi.openApiRoute +
          ") but there is no defined on" +
