...
export const createWebhook  = async(data: WebhookSubscriptionReq):Promise<WebhookSubscriptionRes> => {
    try {
    ...
    const res2 = await runQuery(
        `INSERT INTO ${WEBHOOKS_SUBSCRIPTIONS} (id, user_id, context, table_filter, endpoint , secret_token_link, operation ) VALUES ("${uuid}", "${data.userId}", "${data.context}", "${data.tableFilter}", "${data.endpoint}", "${data.secretTokenLink}", "${data.operation}")`,
    );

    const res3 = await runQuery(`SELECT * FROM ${WEBHOOKS_SUBSCRIPTIONS} WHERE id = '${uuid}'`) as WebhookSubscriptionRes[]
