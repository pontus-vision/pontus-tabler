...
Feature: test jobs
  ...
  Scenario: clean check
    ...
    Then I expect the SQL QUERY "SELECT * FROM delta.`/data/pv_test/jobs_outputs/foo-output`" to have "{"id":"1","name":"foo","create_table":true}"
  ...
  Scenario: Failed job should update status
    ...
    When run a job 1 time(s) with wait 30"
    Then I expect the SQL QUERY "SELECT * FROM delta.`/data/pv_test/jobs_status`" to have "{"status":"failed"}"
  ...
  Scenario: Running no jobs
    ...
    Then I expect the SQL QUERY "SELECT * FROM delta.`/data/pv_test/jobs_status`" to have "[]"
