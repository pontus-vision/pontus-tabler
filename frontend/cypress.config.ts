      ...
      on('task', {
        ...
        async resetDatabaseTablesTest() {  // ✅ No need for an extra Promise wrapper
          ...
          try {
            ...
            const createTable1 = await runQuery('CREATE TABLE IF NOT EXISTS table_1 (id STRING, column_1 STRING, column_2 STRING) USING DELTA LOCATION "/data/pv/table_1";');
            const deleteTable1 = await runQuery('DELETE FROM table_1')
            const createTable2 = await runQuery('CREATE TABLE IF NOT EXISTS table_2 (id STRING, column_1 STRING, column_2 STRING) USING DELTA LOCATION "/data/pv/table_2";');
